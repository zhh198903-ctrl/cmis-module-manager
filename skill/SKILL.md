...
屏蔽位一共**四组**：模块级 `Lower 8-13` → `Lower 31-36`；通道 `11h:134-153` → `10h:213-232`；诊断 `14h:132-149` → `13h:206-223`；**激光器调谐 `12h:231-238` → `12h:239-246`**。

最后一组要单独记两点:**屏蔽位和它管的标志在同一页上**(其余三组都在配对的控制页),而且 Table 8-109 给它每一个位都写着 **`Default: 1`**——规范里其余每组屏蔽位都默认清零,只有这组默认**屏蔽**。所以一块没人配置过的模块,调谐标志一个都到不了主机,面板会在这些标志旁标 `masked`;一条通道六个全屏蔽时,即使这条通道一片安静也会标 `all masked`——那正是没人会想到去查的时候。
